...
export default function AgentActionBar({ agent, onViewFullLog, onViewDiff, onPause, onIntervene }: AgentActionBarProps) {
  ...
  return (
    <div className="flex gap-2 px-4 pb-3 bg-slate-800/50/50 border-t border-slate-800 pt-2">
      <button
        onClick={onViewFullLog}
        className="text-[10px] px-2.5 py-1.5 bg-slate-800 border border-slate-700 rounded-lg text-slate-400 hover:bg-slate-800/50"
      >
        查看完整日志
      </button>
      {hasDiff && (
        <button
          onClick={onViewDiff}
          className="text-[10px] px-2.5 py-1.5 bg-slate-800 border border-emerald-200 rounded-lg text-emerald-600 hover:bg-emerald-50"
        >
          查看 Diff
        </button>
      )}
