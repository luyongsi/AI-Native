...
const priorityC: Record<string, string> = {
  ...
  P2: 'bg-blue-100 text-blue-700 border-blue-200', P3: 'bg-slate-100 text-slate-600 border-slate-200',
};
const statusL: Record<string, string> = {
  pool: '需求池', designing: '设计中', developing: '开发中', testing: '测试中', releasing: '待发布', done: '已上线',
};
const statusBg: Record<string, string> = {
  pool: 'bg-slate-100 text-slate-500', designing: 'bg-amber-50 text-amber-600',
...
const stagePill: Record<string, string> = { done: 'border-emerald-200 text-emerald-700', in_progress: 'border-blue-200 text-blue-700', waiting: 'border-amber-200 text-amber-700', pending: 'border-slate-200 text-slate-400' };
...
function IconChevronDown() { return <svg className="w-3 h-3 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7"/></svg>; }
  ...
  return (
    <button onClick={onClick}
      className={`text-left p-2.5 rounded-lg border transition-all ${
        isSelected ? 'border-slate-900 ring-1 ring-slate-900/10 bg-slate-50' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50/50'
      }`}>
      <div className="flex items-center gap-1.5 mb-1">
        <AgentStatusDot status={agent.status} />
        <span className="text-[11px] font-medium text-slate-800 truncate">{agent.name}</span>
        <span className="text-[9px] text-slate-400 ml-auto flex-shrink-0">{agent.type}</span>
      </div>
      <div className="flex items-center gap-2 text-[9px] text-slate-400">
        {agent.runtime && <span className="flex items-center gap-0.5 text-slate-500"><IconClock />{agent.runtime}</span>}
...
function AgentWorkspace({ agent, requirement, diffs }: { agent: AgentInfo; requirement: Requirement; diffs: CodeDiff[] }) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 border-b border-slate-100 flex-shrink-0">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-slate-500 truncate">{agent.taskName || '空闲'}</span>
          </div>
      ...
      {/* Tab bar */}
      <div className="flex px-4 border-b border-slate-100 flex-shrink-0 bg-white">
              ...
              <div key={diff.id}>
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-1.5">
                    <IconFile />
                    <span className="text-[11px] font-mono text-slate-600">{diff.file}</span>
                  </div>
                ...
                <div className="space-y-1.5">
                  {Object.entries(activitySummary).map(([type, count]) => (
                    <div key={type} className="flex items-center gap-2 text-[10px]">
                      <span className="text-slate-500 w-14 flex-shrink-0">{typeLabels[type] || type}</span>
            ...
            {allFiles.length > 0 && (
              <Section title="修改文件">
                <div className="space-y-1">
                  {allFiles.map((f) => (
                    <div key={f} className="flex items-center gap-1.5 text-[10px] text-slate-600 py-1 px-2 bg-slate-50 rounded">
...
function StatBadge({ value, color, label }: { value: string; color: 'emerald' | 'red' | 'slate'; label?: string }) {
  const tc = color === 'emerald' ? 'text-emerald-600' : color === 'red' ? 'text-red-500' : 'text-slate-700';
  return (
...
function MiniStat({ value, label, color }: { value: string | number; label: string; color?: 'emerald' | 'red' }) {
  const bg = color === 'emerald' ? 'bg-emerald-50' : color === 'red' ? 'bg-red-50' : 'bg-slate-50';
  const tc = color === 'emerald' ? 'text-emerald-600' : color === 'red' ? 'text-red-500' : 'text-slate-700';
  return (
...
export default function RequirementDetailPanel({ requirement, agents, allDiffs, onClose }: RequirementDetailPanelProps) {
  ...
  return (
    <div className="flex-1 bg-white rounded-xl border border-slate-200 flex flex-col min-h-0 overflow-hidden">
      ...
      <div className="flex-shrink-0 px-4 py-2.5 border-b border-slate-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h3 className="text-xs font-semibold text-slate-800 truncate">{requirement.title}</h3>
          ...
          <div className="flex items-center gap-2 flex-shrink-0 ml-2">
            ...
            <button onClick={onClose} className="p-1 hover:bg-slate-100 rounded-md ml-1"><IconClose /></button>
          </div>
        </div>
