...
function computeMyTasks(
  ...
  const approvedThisWeek = approvals.filter((a) => {
    if (a.decision !== 'pass') return false;
    try {
...
function PipelineWidget({ requirements }: { requirements: Requirement[] }) {
  ...
  return (
    <div className="col-span-2 bg-slate-800 rounded-2xl border border-slate-700 p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-slate-200">实时研发流水线</h2>
        <span className="text-[10px] text-slate-400 bg-slate-800/50 px-2 py-1 rounded">
          实时更新
        </span>
      </div>
        ...
        {stageOrder.map((stage, i) => {
          ...
          return (
            <div key={stage} className="flex-1 flex flex-col">
              ...
              >
                <div
                  className={`text-2xl font-bold ${isOver ? 'text-red-600' : 'text-slate-200'}`}
                >
                  {count}
                </div>
                <div
                  className={`text-xs mt-1 ${isOver ? 'text-red-500 font-medium' : 'text-slate-400'}`}
                >
                  {stageLabels[stage]}
                </div>
                {isOver && (
...
/* ============================================================
   Widget 2: 待我审批
   ============================================================ */
function ApprovalWidget({ approvals }: { approvals: Gate0Approval[] }) {
  ...
  return (
    <div className="bg-slate-800 rounded-2xl border border-slate-700 p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-slate-200">待我审批</h2>
      ...
      {pendingApprovals.length === 0 ? (
        ...
        <div className="space-y-3">
          {pendingApprovals.map((a) => {
            const isOverdue = false; // Gate0Approval has no overdue status yet
            return (
              <div
                key={a.id}
                className={`p-3 rounded-xl border ${isOverdue ? 'border-red-200 bg-red-50/50' : 'border-slate-800 hover:border-slate-700'} transition-colors`}
              >
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  >
                    {a.gate_meta?.label || `Gate ${a.gate_level}`}
                  </span>
                  <span
                ...
                <p className="text-xs font-medium text-slate-200">
                  {a.req_title || a.req_id}
                </p>
                <p className="text-[10px] text-slate-400 mt-0.5">
                  {a.reviewer_name || '-'} · {a.created_at ? new Date(a.created_at).toLocaleDateString('zh-CN') : '-'}
                </p>
                <div className="flex gap-2 mt-2">
                  ...
                  <button className="flex-1 py-1.5 bg-slate-800 border border-slate-700 text-slate-400 text-[10px] font-medium rounded-lg hover:bg-slate-800/50 transition-colors">
                    打回
                  </button>
                </div>
              </div>
      ...
      <button className="w-full mt-3 text-[10px] text-slate-400 hover:text-slate-400 transition-colors py-1">
        查看全部审批 &rarr;
      </button>
    </div>
  );
...
function MyRequirementsWidget({
  ...
  const statusColors: Record<string, string> = {
    pool: 'bg-slate-700 text-slate-400',
  ...
  return (
    <div className="col-span-2 bg-slate-800 rounded-2xl border border-slate-700 p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-slate-200">我的需求</h2>
        <button className="text-[10px] text-slate-400 hover:text-slate-400 bg-slate-800/50 hover:bg-slate-700 px-2 py-1 rounded-lg transition-colors">
          + 新建需求
        </button>
      </div>
      ...
      <div className="space-y-1">
        {myReqs.map((req) => (
          <div
            key={req.id}
            className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-slate-800/50 transition-colors cursor-pointer group"
          >
            ...
            <span
              className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${statusColors[req.status] || 'bg-slate-700 text-slate-400'}`}
            >
      ...
      {myTasks.weeklyStats && (
        <div className="mt-4 pt-3 border-t border-slate-800 flex gap-4 text-[10px] text-slate-400">
          <span>
            本周创建:{' '}
...
function AgentActivityWidget({ agents }: { agents: AgentInfo[] }) {
  ...
  const typeColors: Record<string, string> = {
    ...
    commit: 'text-slate-400 bg-slate-700',
          ...
          return (
            ...
            >
              ...
              {isExpanded && agent.lastActivity.length > 0 && (
                <div className="border-t border-slate-50 px-3 pb-3 pt-2 space-y-2 bg-slate-800/50/50">
                  {agent.lastActivity.map((act, i) => (
                    <div key={i} className="flex gap-3 text-xs">
                      ...
                      <span
                        className={`text-[10px] font-medium px-1.5 py-0.5 rounded flex-shrink-0 ${typeColors[act.type] || 'text-slate-400 bg-slate-700'}`}
                      >
  ...
  return (
    <div className="bg-slate-800 rounded-2xl border border-slate-700 p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="text-[10px] text-slate-400 hover:text-slate-400 transition-colors">
          全屏展开
        </button>
      </div>

      {/* Mini topology using CSS grid positioning */}
      <div className="relative h-64 bg-slate-800/50/50 rounded-xl overflow-hidden border border-slate-800">
        {/* SVG lines for edges */}
        <svg
        ...
        <div className="absolute inset-0" style={{ zIndex: 2 }}>
          {/* Orchestrator */}
          <div className="absolute left-1/2 -translate-x-1/2 top-[8%] bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1 shadow-sm">
          ...
          {/* PRD Agent */}
          <div className="absolute left-[14%] top-[30%] bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1 shadow-sm">
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
              <span className="text-[10px] font-medium text-slate-600">
                PRD Agent
          ...
          {/* UI Agent */}
          <div className="absolute left-1/2 -translate-x-1/2 top-[30%] bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1 shadow-sm">
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
              <span className="text-[10px] font-medium text-slate-600">
                UI Agent
          ...
          {/* Test Agent */}
          <div className="absolute left-[74%] top-[30%] bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1 shadow-sm">
            <div className="flex items-center gap-1.5">
          ...
          {/* Spec Decomposer */}
          <div className="absolute left-[30%] top-[55%] bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1 shadow-sm">
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
              <span className="text-[10px] font-medium text-slate-600">
                Spec Decomposer
          ...
          {/* DevAgent-1 */}
          <div className="absolute left-[4%] top-[75%] bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1 shadow-sm">
            <div className="flex items-center gap-1.5">
          ...
          {/* DevAgent-2 */}
          <div className="absolute left-[30%] top-[75%] bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1 shadow-sm">
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-blue-500" />
              <span className="text-[10px] font-medium text-slate-600">
                DevAgent-2
          ...
          {/* DevAgent-3 */}
          <div className="absolute left-[57%] top-[75%] bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1 shadow-sm">
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-blue-500" />
              <span className="text-[10px] font-medium text-slate-600">
                DevAgent-3
          ...
          {/* CI Agent */}
          <div className="absolute left-[16%] top-[88%] bg-slate-800 border border-red-200 rounded-lg px-2.5 py-1 shadow-sm bg-red-50/30">
            <div className="flex items-center gap-1.5">
