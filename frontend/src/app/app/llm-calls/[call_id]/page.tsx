...
function statusBadge(status: string) {
  ...
  var label =
    status === 'success' ? '鎴愬姛' : status === 'error' ? '澶辫触' : status;
  return (
...
export default function LLMCallDetailPage({
  ...
  useEffect(() => {
    api
      ...
      .then(setCall)
      .catch((err) =>
        setError(err instanceof Error ? err.message : '鑾峰彇璋冪敤璇︽儏澶辫触')
      )
    ...
    return (
      <div className="p-6 max-w-[1200px] mx-auto flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs text-slate-400">鍔犺浇璋冪敤璇︽儏...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="p-6 max-w-[1200px] mx-auto flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs text-slate-400">{error || '调用记录不存在'}</span>
          <button
            onClick={() => router.push('/app/llm-calls')}
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-semibold text-slate-100 flex items-center gap-2">
            <span className="font-mono text-sm text-slate-400">{call.call_id ? call.call_id.slice(0, 12) : '-'}</span>
            {statusBadge(call.status)}
          </h1>
          <p className="text-xs text-slate-400 mt-0.5">LLM 璋冪敤璇︽儏</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 space-y-3">
            <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">璋冪敤淇℃伅</h2>
            ...
            <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Token 鐢ㄩ噺</h2>
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                <span className="text-slate-400">Prompt</span>
                <span className="font-mono text-slate-600">{call.prompt_tokens.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-slate-400">Completion</span>
                <span className="font-mono text-slate-600">{call.completion_tokens.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-xs font-semibold">
                <span className="text-slate-600">Total</span>
                <span className="font-mono text-slate-100">{tokenTotal.toLocaleString()}</span>
              </div>
            ...
            <div className="flex justify-between text-xs">
              <span className="text-slate-400">Prompt chars</span>
              <span className="font-mono text-slate-600">{call.prompt_chars.toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-slate-400">Response chars</span>
              <span className="font-mono text-slate-600">{call.response_chars.toLocaleString()}</span>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800 bg-slate-800/50/50">
              <h2 className="text-xs font-semibold text-slate-600">
                Prompt ({messages.length} messages)
              </h2>
              <button
                onClick={() => setShowRawPrompt(!showRawPrompt)}
                className="text-[10px] px-2 py-1 rounded border border-slate-700 text-slate-400 hover:bg-slate-700"
              >
            ...
            <div className="p-4 max-h-[600px] overflow-y-auto">
              {showRawPrompt ? (
                <pre className="text-xs font-mono text-slate-600 whitespace-pre-wrap break-all bg-slate-800/50 rounded-lg p-3 max-h-[500px] overflow-auto">
                  {call.prompt || '(empty)'}
                </pre>
              ) : (
                <div className="space-y-3">
                  {messages.map((msg, i) => (
                    <div key={i} className="rounded-lg border border-slate-800 overflow-hidden">
                      <div className="px-3 py-1.5 bg-slate-800/50 border-b border-slate-50">
                        <span className={'text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded ' + roleClass(msg.role)}>
                          {msg.role}
                        </span>
                      </div>
                      <pre className="p-3 text-xs text-slate-600 whitespace-pre-wrap break-all">
                        {typeof msg.content === 'string' ? msg.content : JSON.stringify(msg.content, null, 2)}
                      </pre>
                    </div>
                  ))}
                  {messages.length === 0 && (
                    <div className="text-xs text-slate-400 py-8 text-center">鏃?Prompt 鍐呭</div>
                  )}
                </div>
              )}
            </div>
          ...
          {/* Response */}
          <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-800 bg-slate-800/50/50">
              <h2 className="text-xs font-semibold text-slate-600">Response</h2>
            </div>
            <div className="p-4">
              {call.response ? (
                <pre className="text-xs font-mono text-slate-600 whitespace-pre-wrap break-all bg-slate-800/50 rounded-lg p-3 max-h-[600px] overflow-y-auto">
                  {call.response}
                </pre>
              ) : (
                <div className="text-xs text-slate-400 py-8 text-center">
                  {call.status === 'error' ? '璋冪敤澶辫触锛屾棤鍝嶅簲鍐呭' : '鏃?Response 鍐呭'}
                </div>
              )}
            </div>
          </div>
          ...
          {call.response_preview && call.response && call.response.length > 500 && (
            <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-800 bg-slate-800/50/50">
                <h2 className="text-xs font-semibold text-slate-600">Response Preview (鍓?500 瀛楃)</h2>
              </div>
              <div className="p-4">
                <pre className="text-xs font-mono text-slate-400 whitespace-pre-wrap break-all bg-slate-800/50 rounded-lg p-3">
  ...
  return (
    <div className="flex justify-between items-start gap-2">
      ...
      {badge ? (
        ...
        <span className={'text-xs text-slate-600 text-right break-all ' + (mono ? 'font-mono text-[11px]' : '')}>
          {value}
        </span>
      )}
    </div>
