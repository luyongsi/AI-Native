...
'ring-2 ring-slate-900 ring-offset-1' : ''}`}
                        >
                          ...
                          {req.stages && req.stages.find((s: any) => s.status === 'in_progress') && (
            ...
            <tbody>
              {requirements.map((req) => (
                <tr key={req.id} className="border-b border-slate-100 hover:bg-slate-50 cursor-pointer" onClick={() => setSelectedReq(req)}>
                  ...
                  <td className="py-2.5 px-3 text-slate-400">{req.createdAt || req.created_at}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="fixed right-0 top-12 bottom-8 w-[420px] bg-white border-l border-slate-200 z-50 shadow-xl overflow-y-auto">
            <div className="p-5">
              ...
              <div className="flex gap-2 mb-4">
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded ${priorityColors[selectedReq.priority] || 'bg-slate-100 text-slate-600'}`}>{selectedReq.priority}</span>
