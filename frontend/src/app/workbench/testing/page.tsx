  ...
  if (testableReqs.length === 0) {
    return (
      <div className="p-6">
        {/* Page Header */}
        <div className="mb-6">
          <h1 className="text-lg font-semibold text-slate-900">测试工作台</h1>
          <p className="text-xs text-slate-500 mt-1">选择一个需求，进入测试工作台进行用例编写与执行</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            <h2 className="text-xs font-semibold text-slate-800">可测需求列表</h2>
            <div className="flex items-center gap-2">
              <input
                placeholder="搜索需求..."
                className="text-xs border border-slate-200 rounded-lg px-3 py-1.5 outline-none focus:border-slate-400 w-48"
              />
            </div>
          </div>
        ...
        <div className="divide-y divide-slate-50">
          {testableReqs.map((req) => {
            ...
            return (
              <button
                key={req.id}
                onClick={() => router.push(`/workbench/testing/${req.id}`)}
