    ...
    return (
      <div className="p-6 max-w-[1600px] mx-auto h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xs text-slate-400">加载告警数据中...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-lg font-semibold text-slate-900">告警中心</h1>
          <p className="text-xs text-slate-500 mt-0.5">
            活跃告警: {activeAlerts} · 今天已解决: 5 · 7天内: 23
          </p>
        </div>
        <button className="px-4 py-2 bg-white border border-slate-200 text-xs font-medium text-slate-600 rounded-lg hover:bg-slate-50">
          配置告警规则
        </button>
      </div>
        ...
        {(['all', 'critical', 'warning'] as const).map((f) => (
          <button
            ...
            className={`text-[10px] px-3 py-1.5 rounded-lg transition-colors ${
              filter === f ? 'bg-slate-900 text-white' : 'text-slate-500 bg-white border border-slate-200 hover:bg-slate-50'
            }`}
          >
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  <button className="p-1.5 hover:bg-slate-200 rounded-lg text-slate-400" title="确认">
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                  </button>
                  <button className="p-1.5 hover:bg-slate-200 rounded-lg text-slate-400" title="静默">
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <svg className={`w-3 h-3 text-slate-300 transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div className="border-t border-slate-100 px-4 py-4 bg-slate-50/50">
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <div className="text-xs text-slate-700 font-mono bg-white border border-slate-100 rounded-lg p-2.5">
                        {alert.rootCause}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400 mb-1">AI 建议</div>
                      <div className="text-xs text-slate-700 bg-white border border-slate-100 rounded-lg p-2.5">
                        {alert.aiSuggestion || '暂无建议'}
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button className="px-3 py-1.5 bg-white border border-slate-200 text-xs text-slate-600 rounded-lg hover:bg-slate-50">
                      查看详细日志
                    </button>
      ...
      {/* Alert Rules Section */}
      <div className="mt-8">
        <h2 className="text-sm font-semibold text-slate-900 mb-4">告警规则配置</h2>
...
function RuleCategory({ title, rules }: { title: string; rules: { name: string; level: string; enabled: boolean }[] }) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4">
      <h3 className="text-xs font-semibold text-slate-800 mb-3">{title}</h3>
      <div className="space-y-2">
        {rules.map((rule, i) => (
          <div key={i} className="flex items-center gap-2 text-xs">
            ...
            <span className="text-slate-700 flex-1">{rule.name}</span>
