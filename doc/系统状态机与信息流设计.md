...
| 产出物 | 存储位置 |
|--------|---------|
| 需求草案（标题、描述、实体、用例） | `requirements` 表 |
| 验收标准（GWT 格式） | `spec.artifacts.A1` |
| 低保真线框图 URL（必要时） | `spec.artifacts.A1` |
...
| 产出物 | 存储位置 |
|--------|---------|
| 可行性评估（技术/业务/风险级别） | `spec.artifacts.A2` |
| 待确认清单 | `spec.artifacts.A2` |
| 冲突点列表（关联系统、冲突描述） | `spec.artifacts.A2` |
...
| 产出物 | 存储位置 |
|--------|---------|
| Spec 文档（含状态机） | `spec.artifacts.A4` → `requirements.spec` 根键 |
| OpenAPI Schema | `spec.artifacts.A4` |
| ERD 实体关系图 | `spec.artifacts.A4` |
| DDL SQL 语句 | `spec.artifacts.A4` |
...
| 产出物 | 存储位置 |
|--------|---------|
| 设计检查报告（API一致性、ERD字段完整性、状态机闭合性等） | `spec.artifacts.A5` |
...
| 产出物 | 存储位置 |
|--------|---------|
| 架构设计文档 | `spec.artifacts.A6` |
| 任务拆分 DAG（节点 ID、标题、依赖边、预估工时） | `spec.artifacts.A6` |
...
| 产出物 | 存储位置 |
|--------|---------|
| 测试用例骨架（单元/集成/E2E） | `spec.artifacts.A7` |
...
| 产出物 | 存储位置 |
|--------|---------|
| 代码 diff 摘要 | `spec.artifacts.A9` |
| commit 信息 | `spec.artifacts.A9` |
...
| 产出物 | 存储位置 |
|--------|---------|
| 部署状态 | `spec.artifacts.A10` |
| Sandbox URL | `spec.artifacts.A10` |
...
| 产出物 | 存储位置 |
|--------|---------|
| 测试报告（总数/通过/失败/跳过/覆盖率） | `spec.artifacts.A11` |
...
| 产出物 | 存储位置 |
|--------|---------|
| 审查报告（问题列表+严重度+建议） | `spec.artifacts.A12` |
...
| `agent.result.gate0.reject` | Gate → Orchestrator + A1 | Gate0 拒绝 | 《Gate0审批意见》（拒绝原因 + 分类/严重度 + 修订指引） |
