...
class FastChannelClassifier(BaseAgentWorker):
    ...
    async def _call_llm(self, messages: list, temperature: float = 0.1) -> str | None:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                resp = await client.post(
                    ...
                    json={"model": DEEPSEEK_MODEL, "messages": messages, "temperature": temperature, "max_tokens": 1000},
                )
                resp.raise_for_status()
                return resp.json()["choices"][0]["message"]["content"]
        except Exception as e:
            logger.error(f"[FC] LLM call failed: {e}")
            return None
    ...
    async def execute(self, req_id: str, context_package: dict) -> dict:
        ...
        complexity_4 = await self._llm_complexity(title, description, domain)
    ...
    async def _llm_complexity(self, title: str, description: str, domain: str) -> float:
        """LLM 评估需求复杂度 (0-100)"""
